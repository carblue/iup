 ...
 */
module im.im_capture;

version(IM) :
version(Windows) :
version(D_LP64) :

version(DigitalMars) { pragma(lib, "im_capture.lib"); } // found in Win64 package only
version(DigitalMars) { pragma(lib, "strmiids.lib"); } // link with the "strmiids.lib" library on Windows. Visual C ?
...
//extern(C++) there doesn't seem to be interaction with C++ -code, thus it may have D linkage
deprecated("USE NAMESPACE BASED CLASSES.")
class imCapture
{
public:
  this() 
