 ...
 */
module im.im_process_pnt;

version(IM) :

import im.im : FreeEnumMembers;
...
mixin FreeEnumMembers!imUnaryOp;
...
mixin FreeEnumMembers!imBinaryOp;
...
mixin FreeEnumMembers!imLogicOp;
...
mixin FreeEnumMembers!imToneGamut;
...
mixin FreeEnumMembers!imToneGamutFlags;
...
void imImageGamma(imImage* _image, float _gamma) { float[1] params; params[0] = _gamma; imProcessToneGamut(_image, _image, IM_GAMUT_POW, params.ptr); }
 ...
 * \ingroup imageenhance */
void imImageBrightnessContrast(imImage* _image, float _bright_shift, float _contrast_factor) { float[2] _params; _params[0] = _bright_shift; _params[1] = _contrast_factor; imProcessToneGamut(_image, _image, IM_GAMUT_BRIGHTCONT, _params.ptr); }

/** Same as \ref imProcessToneGamut using \ref IM_GAMUT_EXPAND.
 *
 * \verbatim image:Level(start, end)  [in Lua 5] \endverbatim
 * \ingroup imageenhance */
void imImageLevel(imImage* _image, float _start, float _end) { float[2] _params; _params[0] = _start; _params[1] = _end; imProcessToneGamut(_image, _image, IM_GAMUT_EXPAND, _params.ptr); }

/** Same as \ref imProcessEqualizeHistogram.
 *
 * \verbatim image:Equalize()  [in Lua 5] \endverbatim
 * \ingroup imageenhance */
void imImageEqualize(imImage* _image) { imProcessEqualizeHistogram(_image, _image); }
 ...
 * \ingroup imageenhance */
void imImageNegative(imImage* _image) { imProcessNegative(_image, _image); }

/** Same as \ref imProcessExpandHistogram.
 *
 * \verbatim image:AutoLevel(percent)  [in Lua 5] \endverbatim
 * \ingroup imageenhance */
void imImageAutoLevel(imImage* _image, float _percent) { imProcessExpandHistogram(_image, _image, _percent); }
