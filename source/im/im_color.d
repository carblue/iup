...
//#include "im_math.h" //import im.im_math;
 ...
 * \ingroup color */
float imColorZeroShift(int data_type) pure nothrow @nogc @safe // we want -boundscheck in release code
{
  float[8] zero= [128.0f,     // [-128,+127]
                  0,
 ...
 * See \ref cci.
 * \ingroup color */
int imColorMax(int data_type) pure nothrow @nogc @safe // we want -boundscheck in release code
{
  int[8] max  = [255,
                 32767,
                 65535,
 ...
 * See \ref cci.
 * \ingroup color */
int imColorMin(int data_type) pure nothrow @nogc @safe // we want -boundscheck in release code
{
  int[8] min = [0,
                -32768,
                0,
 ...
 * See also \ref math.
 * \ingroup color */
T imColorQuantize(T)(ref const(float) value, ref const(T) min, ref const(T) max) pure nothrow @nogc @safe
	if (isNumeric!T)
{
  if (max == 1) return cast(T)value; // to allow a dummy quantizer
  if (value >= 1) return max;
  if (value <= 0) return min;
  float range = cast(float)max - cast(float)min + 1.0f;
  return cast(T)imRound(value*range - 0.5f) + min;
}
 ...
 * See also \ref math.
 * \ingroup color */
float imColorReconstruct(T)(ref const(T) value, ref const(T) min, ref const(T) max) pure nothrow @nogc @safe
	if (isNumeric!T)
{
  if (max == 1) return cast(float)value;  // to allow a dummy reconstructor
  if (value <= min) return 0;
  if (value >= max) return 1;
  float range = cast(float)max - cast(float)min + 1.0f;
  return ((cast(float)value - cast(float)min + 0.5f)/range);
}
...
\endverbatim
 * \ingroup color */
void imColorYCbCr2RGB(T)(const T Y, const T Cb, const T Cr,
                             ref T R, ref T G, ref T B,
                             ref const(T) zero, ref const(T) min, ref const(T) max) pure nothrow @nogc @safe
	if (isNumeric!T)
{
  ...
  R = cast(T)IM_CROPMINMAX(r, min, max);
  G = cast(T)IM_CROPMINMAX(g, min, max);
  B = cast(T)IM_CROPMINMAX(b, min, max);
}
...
\endverbatim
 * \ingroup color */
void imColorRGB2YCbCr(T)(const T R, const T G, const T B,
                             ref T Y, ref T Cb, ref T Cr,
                             ref const(T) zero) pure nothrow @nogc @safe
	if (isNumeric!T)
{
  Y  = cast(T)( 0.299f *R + 0.587f *G + 0.114f *B);
  Cb = cast(T)(-0.169f *R - 0.331f *G + 0.500f *B + cast(float)zero);
  Cr = cast(T)( 0.500f *R - 0.419f *G - 0.081f *B + cast(float)zero);
}
...
\endverbatim
 * \ingroup color */
void imColorCMYK2RGB(T)(const T C, const T M, const T Y, const T K,
                            ref T R, ref T G, ref T B, ref const(T) max) pure nothrow @nogc @safe
	if (isNumeric!T)
{
  T W = max - K;
  R = cast(T)((W * (max - C)) / max);
  G = cast(T)((W * (max - M)) / max);
  B = cast(T)((W * (max - Y)) / max);
}
...
\endverbatim
 * \ingroup color */
void imColorXYZ2RGB(T)(const T X, const T Y, const T Z,
                           ref T R, ref T G, ref T B) pure nothrow @nogc @safe
	if (isNumeric!T)
{
  ...
  R = cast(T)IM_FLOATCROP(r);
  G = cast(T)IM_FLOATCROP(g);
  B = cast(T)IM_FLOATCROP(b);
}
...
\endverbatim
 * \ingroup color */
void imColorRGB2XYZ(T)(const T R, const T G, const T B,
                           ref T X, ref T Y, ref T Z) pure nothrow @nogc @safe
	if (isNumeric!T)
{
  X = cast(T)(0.4124f *R + 0.3576f *G + 0.1805f *B);
  Y = cast(T)(0.2126f *R + 0.7152f *G + 0.0722f *B);
  Z = cast(T)(0.0193f *R + 0.1192f *G + 0.9505f *B);
}

float IM_FWLAB(float _w) nothrow @nogc @safe { // powf turns it impure
                        return (_w > 0.008856f?
                        powf(_w, 1.0f/3.0f):
...
\endverbatim
 * \ingroup color */
void imColorXYZ2Lab(const float X, const float Y, const float Z,
                          ref float L, ref float a, ref float b) nothrow @nogc @safe
{
...
float IM_GWLAB(float _w) nothrow @nogc @safe  // powf turns it impure
{                        return (_w > 0.20689f?
                         powf(_w, 3.0f):
 ...
 * 0 <= L <= 1 ; -0.5 <= ab <= +0.5 ; 0 <= XYZ <= 1
 * \ingroup color */
void imColorLab2XYZ(const float L, const float a, const float b,
                          ref float X, ref float Y, ref float Z) nothrow @nogc @safe
...
\endverbatim
 * \ingroup color */
void imColorXYZ2Luv(const float X, const float Y, const float Z,
                          ref  float L, ref float u, ref float v) nothrow @nogc @safe
{
  float XYZ = cast(float)(X + 15 * Y + 3 * Z);
  float fY = Y / 1.0f;
 ...
 * \ingroup color */
void imColorLuv2XYZ(const float L, const float u, const float v,
                          ref float X, ref float Y, ref float Z) nothrow @nogc @safe
...
\endverbatim
 * \ingroup color */                           
float imColorTransfer2Linear(ref const(float) nonlinear_value) nothrow @nogc @safe
{
...
\endverbatim
 * \ingroup color */                           
float imColorTransfer2Nonlinear(ref const(float) value) nothrow @nogc @safe
{
  if (value < 0.0031308f)
    return 12.92f * value;
  else
 ...
 * \ingroup color */
void imColorRGB2RGBNonlinear(const float RL, const float GL, const float BL,
                                   ref float R, ref float G, ref float B) nothrow @nogc @safe
{
...
\endverbatim
 * \ingroup color */
T imColorRGB2Luma(T)(const T R, const T G, const T B) pure nothrow @nogc @safe
	if (isNumeric!T)
{
  return cast(T)((299 * R + 587 * G + 114 * B) / 1000);
}
...
\endverbatim
 * \ingroup color */
float imColorLuminance2Lightness(ref const(float) Y) nothrow @nogc @safe
{
...
\endverbatim
 * \ingroup color */
float imColorLightness2Luminance(ref const(float) L) nothrow @nogc @safe
{
  float fY = (L + 0.16f) / 1.16f;
  return IM_GWLAB(fY);
}
